...
function sideBySideImage(punkId) {
    return `https://storage.googleapis.com/hdpunks-cdn/sidebyside/${punkId}.png`;
}
...
async function populateReservationCount() {
    let reservationsLeft = await punksRemaining();
    $("#reservationCount").text(reservationsLeft);
}

async function populateMintCount() {
    let mintCount = await hdContract.methods.numTokens().call();
    let mintPercentage = Math.round(1000 * mintCount / 5000) / 10;
    console.log(`mintCount ${mintCount}, mintPercentage ${mintPercentage}`);
    $("#mintCount").text(mintCount);
    $("#mintPercentage").text(mintPercentage + "%");

}

async function reserveFeeInWei() {
    let fee = await reservationContract.methods.price().call();
    return fee;
}
...
async function punksRemaining() {
    let total = await reservationContract.methods.maxPunks().call();
    let minted = await reservationContract.methods.mintCount().call();
    return total - minted;
}

async function reserveButtonClick() {
    let quantity = $("#quantity").val();
    console.log(quantity);
    if (quantity > 49) {
        quantity = 49;
    }
    await reserve(quantity);
}

async function mintRandomButtonClick() {
    let quantity = $("#quantity").val();
    await mintRandom(quantity);
}
...
async function reserve(quantity) {
    let account = await getAccount();
    let fee = web3.utils.toBN(await reserveFeeInWei()).mul(web3.utils.toBN(quantity));
    let ethFee = web3.utils.fromWei(fee, "ether");
    console.log(`Calling web3 mint(${account}, ${quantity}), mint fee ${ethFee}`);
    let tx = await reservationContract.methods.mint().send({
        "from": account,
        "value": fee
    });
    console.log(tx);
...
async function mintRandom(quantity) {
    let account = await getAccount();
    let fee = web3.utils.toBN(await mintFeeInWei()).mul(web3.utils.toBN(quantity));
    let ethFee = web3.utils.fromWei(fee, "ether");
    console.log(`Calling web3 mint(${account}, ${quantity}), mint fee ${ethFee}`);
    let tx = await hdContract.methods.mintRandom(account, quantity).send({
        "from": account,
        "value": fee
    });
    console.log(tx);
...
async function populateSearchId() {
    ...
    console.log(punkId);
    $("#searchSideBySidePunkImage").attr("src", sideBySideImage(punkId));
}

async function randomSearchId() {
    let punkId = parseInt((Math.random() * 10000), 10)
    $("#searchId").val(punkId);
...
window.onload = async function() {
    ...
    await populateReservationCount();
